...
fetched: 2026-06-21T06:40:15.000783
---
...
deploymentdockerdocker-composeinfrastructureOn this pageRailway does not run `docker-compose.yml` files directly. Instead, each service defined in your Compose file maps to a separate Railway service within a project. The sections below walk through that translation so you can take an existing Compose setup and deploy it to production without managing servers.


## [How Compose maps to Railway](#how-compose-maps-to-railway)
...
## [1. Create a Railway project](#1-create-a-railway-project)


Open the [Railway dashboard](https://railway.com/dashboard) and click **+ New Project**. Choose **Empty project**. This project will hold all of the services that correspond to your Compose file.


## [2. Add services](#2-add-services)


Work through each entry under `services:` in your Compose file and create a matching Railway service.


### [Services with a Dockerfile or build context](#services-with-a-dockerfile-or-build-context)
...
### [Services using a Docker image](#services-using-a-docker-image)
...
* Enter the image name and tag, such as `redis:7-alpine`.
...
## [4. Set up networking](#4-set-up-networking)
...
## [5. Attach volumes](#5-attach-volumes)
...
* Set the **Mount Path** to match the path in your Compose file (for example, `/var/lib/postgresql/data`).
...
* If you generated a public domain, open it in a browser or send a request with `curl` to verify the service responds.

* For services that depend on a database, confirm the connection by checking logs for successful queries or running a health check endpoint.


## [Example: translating a Compose file](#example-translating-a-compose-file)
...
[How Compose maps to Railway](#how-compose-maps-to-railway)[Prerequisites](#prerequisites)[1. Create a Railway project](#1-create-a-railway-project)[2. Add services](#2-add-services)[Services with a Dockerfile or build context](#services-with-a-dockerfile-or-build-context)[Services using a Docker image](#services-using-a-docker-image)[Database services](#database-services)[3. Configure environment variables](#3-configure-environment-variables)[4. Set up networking](#4-set-up-networking)[Private networking](#private-networking)[Public networking](#public-networking)[5. Attach volumes](#5-attach-volumes)[6. Verify the deployment](#6-verify-the-deployment)[Example: translating a Compose file](#example-translating-a-compose-file)[Next steps](#next-steps)[Edit this page on GitHub](https://github.com/railwayapp/docs/edit/main/content/guides/docker-compose.md)Last updated Jun 21, 2026